...
function OfficeCanvasFallback({
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(251,191,36,0.12),_transparent_48%),_#120b06] px-6 text-center">
      <div className="max-w-xl rounded-2xl border border-amber-900/30 bg-[#1a1008]/90 p-6 shadow-2xl backdrop-blur-sm">
        <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-amber-500/70">
          Retro Office fallback
        </div>
        <h2 className="mt-3 text-xl font-semibold text-amber-100">{title}</h2>
        <p className="mt-3 text-sm leading-6 text-amber-50/75">{body}</p>
        <div className="mt-4 text-xs text-amber-200/60">
          Tip: add <span className="font-semibold text-amber-100">?no3d=1</span>{" "}
          to the office URL to disable the canvas during desktop stability testing.
        </div>
        {showRetry && onRetry ? (
          <button
            type="button"
            onClick={onRetry}
            className="mt-5 inline-flex items-center justify-center rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-2 text-sm font-medium text-amber-100 transition-colors hover:bg-amber-500/20"
          >
            Retry 3D canvas
          </button>
        ) : null}
...
export function RetroOffice3D({
  ...
  const [deferHeavySceneContent, setDeferHeavySceneContent] = useState(true);
  const [deferEnvironment, setDeferEnvironment] = useState(true);
  ...
  useEffect(() => {
    if (disable3D || webglContextLost) {
      setDeferHeavySceneContent(true);
  ...
  const retry3DCanvas = useCallback(() => {
    setWebglContextLost(false);
    setCanvasInstanceKey((current) => current + 1);
  }, []);

  const showCanvasFallback = disable3D || webglContextLost;
  ...
  return (
    <div className="relative w-full h-full bg-[#1a1008] font-mono text-white overflow-hidden">
      ...
      >
        ...
        */}
        {!immersiveOverlayActive && !showCanvasFallback ? (
          <Canvas
            key={canvasInstanceKey}
            fallback={
              <OfficeCanvasFallback
                title="3D rendering is unavailable in this browser"
                body="The office can still run without WebGL. Use ?no3d=1 for a simple non-3D debugging session, or retry once graphics acceleration is available again."
              />
            }
            orthographic
            dpr={[0.8, 1.25]}
            ...
            onCreated={({ gl }) => {
              ...
              const handleContextLost = (event: Event) => {
                ...
                console.warn("RetroOffice3D WebGL context lost; switching to fallback UI.");
          ...
          >
            ...
            {/* Environment lighting — defer until the base scene is stable. */}
            {!deferEnvironment ? (
              <Suspense fallback={null}>
            ...
            {/* Furniture models — keep first paint light, then load the heavier pieces. */}
            <Suspense fallback={null}>
              {!editMode ? (
                <PrimitiveInstancedWallSegmentsModel items={wallItems} />
              ) : null}
            ...
            {!deferHeavySceneContent && remoteLayoutFurniture.length > 0 ? (
          ...
          {/* Removed standalone Jukebox as it's now in the furniture loop */}
            ...
            {!deferHeavySceneContent ? sceneAgents.map((agent) => {
              const isJanitor = "role" in agent && agent.role === "janitor";
              return (
                <AgentObjectModel
                  key={agent.id}
        ...
        {!immersiveOverlayActive && showCanvasFallback ? (
          <OfficeCanvasFallback
            title={
              disable3D
                ? "3D canvas disabled for this session"
                : "The 3D canvas lost its WebGL context"
            }
            body={
              disable3D
                ? "You are in the safer non-3D office mode for debugging. Remove ?no3d=1 from the URL whenever you want to bring the full canvas back."
                : "The office switched to a non-3D fallback instead of leaving a broken canvas on screen. You can retry the renderer once, or keep using ?no3d=1 while testing on lower-spec hardware."
            }
            onRetry={disable3D ? undefined : retry3DCanvas}
            showRetry={!disable3D}
          />
        ) : null}
      </div>
